...
  // Helper function to count borrows for a single book
  function countBorrows(acc, book) {
    return acc + book.borrows.filter(borrow => borrow.id === account.id).length;
  }

  // Using reduce to iterate through books and count borrows
  return books.reduce(countBorrows, 0);
}
